...
export async function newUserService(
  ...
  const emailExists = allUsers.find((user) => user.email === newUserData.email);
  ...
  const { data: savedNewUser, error: savedNewUserError } = await wrapInPromise(
    newUser.save(),
  );

  if (!savedNewUser || savedNewUserError) {
    throw new Error(
      "Error while saving new user to database: " + savedNewUserError.message,
      { cause: newUser },
    );
  }
